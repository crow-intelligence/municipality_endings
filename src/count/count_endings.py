...
with open("data/raw/endings_edited.txt", "r") as infile:
...
with open("data/processed/suffix_counts.tsv", "w") as outfile:
...
def suffix_categorizer(m):
    for k in list(suffix_counts.keys())[:20]:
        if m.endswith(k):
            return k
    return "other"
...
m_category = {}
with open("data/processed/municipality_endings.tsv", "w") as outfile:
    h = "Municipality\tending\tpopulation\n"
    outfile.write(h)
...
with open("data/processed/catgegory_count_population.tsv", "w") as outfile:
